...
Widget buildView(OnboardPage3State state, Dispatch dispatch, ViewService viewService) {
  final TextStyle _title = TextStyle(fontWeight: FontWeight.w800, fontSize: 26);
  final TextStyle _subTitle = TextStyle(fontWeight: FontWeight.w200, fontSize: 14, color: Colors.grey);
          ...
          child: Column(
              ...
              children: [
                ...
                SizedBox(
                    ...
                    child: ElevatedButton(onPressed: () => dispatch(OnboardingActionCreator.moveToPage(4)), child: Text('Next'))),
...
class __TrackerBubblesState extends State<_TrackerBubbles> {
  ...
  __TrackerBubblesState(ThirdPartyTrackersEnum tracker): _image = mapTrackerToAsset(tracker);
  ...
  void _update() {
    switch(widget.tracker) {
      case ThirdPartyTrackersEnum.anilist:
        ...
        break;
      case ThirdPartyTrackersEnum.myanimelist: this.setState(() {
        _avatar = GlobalUserStore.store.getState().myanimelistUser?.avatar;
        _userName = GlobalUserStore.store.getState().myanimelistUser?.username;
      });
      break;
      case ThirdPartyTrackersEnum.simkl: this.setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () {
        if (_userName == null)
        switch(widget.tracker) {
          case ThirdPartyTrackersEnum.anilist: AnilistAPI().login().whenComplete(() {
            AnilistAPI().getProfile().whenComplete(() => _update());
          }); break;
          case ThirdPartyTrackersEnum.simkl: SimklAPI().login().whenComplete(() => SimklAPI().getProfile().whenComplete(() => _update())); break;
          case ThirdPartyTrackersEnum.myanimelist: MyAnimeListAPI().login().whenComplete(() => MyAnimeListAPI().getProfile().whenComplete(() => _update()));
        }
      },
      child: Container(
        ...
        child: Column(
          children: [
            CircleAvatar(
              backgroundImage: _avatar != null ? (NetworkImage(_avatar!) as ImageProvider<Object>?) : (AssetImage(_image)),
              radius: 60,
            ),
            Padding(
              padding: const EdgeInsets.only(top: 10.0),
              child: Text(_userName ?? '', style: TextStyle(fontWeight: FontWeight.w700)),
            ),
          ],
        ),
      ),
