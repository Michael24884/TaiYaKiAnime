...
const VideoPlayerScreen: FC<Props> = (props) => {
	...
	const lastQualityPref = useRef<string>();
	const lastServerPref = useRef<string>();
	...
	useEffect(() => {
		if (error) OrientationLocker.lockToPortrait();
	...
	useEffect(() => {
		...
		if (
			...
			const findLast = preloadedVideoRef.current.links.find(
				(i) => i.server === lastServerPref.current
			);
			if (findLast) setCurrentServer(findLast);
			else setCurrentServer(preloadedVideoRef.current.links[0]);
	...
	const scrapeLinks = async (
		...
		try {
			...
			if (!isRef) {
				...
				const findLast = filteredList.find(
					(i) => i.server === lastServerPref.current
				);
				if (findLast) setCurrentServer(findLast);
				else setCurrentServer(filteredList[0]);
			} else {
				preloadedVideoRef.current = {
					episode: refEpisode!,
					links: filteredList,
	...
	useEffect(() => {
		...
		const findServer = async () => {
			try {
				...
				//TODO: Look for previous references
				const findLast = servers.find(
					(i) => i.quality === lastQualityPref.current
				);
				if (findLast) setCurrentQuality(findLast);
				else setCurrentQuality(servers[0]);
	...
	const saveHistory = () => {
		...
		getItem()
			...
			.finally(widgetHandler.getLatestHistory);
	};
		...
		return (
			<TouchableOpacity
				onPress={() => {
					...
					lastQualityPref.current = item.quality;
	...
	const _renderServers = ({
		item,
	}: {
		item: { link: string; server: string };
	}) => {
		return (
			<TouchableOpacity
				onPress={() => {
					setCurrentServer(item);
					lastServerPref.current = item.server;
				...
				>
					{scrapingProgress === "ERROR" || error ? (
						<View style={styles.errorView}>
							...
							<ThemedText>
								{(error?.toString() ?? "Reason unknown").toString()}
							</ThemedText>
						</View>
				...
				{isFullScreen ? null : (
					...
					>
						...
						>
							...
							<View
								style={{ backgroundColor: "grey", width: 2, height: "100%" }}
							/>
							<Icon
								name={"cog"}
								type={"MaterialCommunityIcons"}
